...
SET @IMAGE_BASE_URL = "https://movie-1258005796.cos.ap-guangzhou.myqcloud.com/";
...
CREATE TABLE `movies_comment` (
  ...
  `user_name` varchar(16) NOT NULL comment '评论者名称',
  `user_image` varchar(128) NOT NULL comment '评论者头像',
  `user_id` varchar(128) NOT NULL comment '评论者id',
  `content` TEXT DEFAULT NULL comment '电影评论的文本内容',
  `mp3_url` varchar(128) DEFAULT NULL comment '电影评论的音频地址链接',
  `type` int(1) NOT NULL DEFAULT 0 comment '评论的类型，0表示文本，1表示音频',
  `comment_time` datetime NOT NULL DEFAULT CURRENT_TIMESTAMP comment '电影评论时间', 
  `recording_duration` int(11) NOT NULL DEFAULT 0 comment '录音时间',
  PRIMARY KEY (`id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8;

CREATE TABLE `comment_collection` (
  `id` int(11) NOT NULL AUTO_INCREMENT,
  `user_id` varchar(128) NOT NULL comment '收藏者id',
  `comment_id` int(11) NOT NULL comment '评论id',
  `collection_time` datetime NOT NULL DEFAULT CURRENT_TIMESTAMP comment '影评收藏时间', 
  PRIMARY KEY (`id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8;